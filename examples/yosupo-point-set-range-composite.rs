...
fn main() {
    // My Modint struct needs rustc 1.51.0 ..... Ooops
    let (n, q) = get!(usize, usize);
    let l = get!(usize,usize;n);
    let l: Vec<(usize, usize)> = l.into_iter().map(|x| (x.0, x.1)).collect();

    let query = get!(usize,usize,usize,usize; q);

    let e: (usize, usize) = (1, 0);
    let mut seg = Segtree::new(&l, e, |x, y| {
        (
            (x.0 * y.0) % 998244353,
            ((x.1 * y.0) % 998244353 + y.1) % 998244353,
        )
    });
    for q in query {
        if q.0 == 0 {
            ...
            let (a, b) = seg.prod(q.1, q.2);
            // println!("a : {} , b : {}", a, b);
            let x: usize = q.3;
            println!("{}", ((a * x) % 998244353 + b) % 998244353);
        }
    }
}
